...
@section('page-title', 'Apri Ticket')
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <textarea name="descrizione" class="form-control" rows="4" required>{{ old('descrizione') }}</textarea>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="categoria" class="form-select" required>
                        ...
                        @foreach($cats as $key=>$label)
                            <option value="{{ $key }}" {{ old('categoria', $precompilazione['categoria'])===$key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="priorita" class="form-select" required>
                        @foreach(['bassa'=>'Bassa','media'=>'Media','alta'=>'Alta','critica'=>'Critica'] as $key=>$label)
                            <option value="{{ $key }}" {{ old('priorita')===$key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="urgenza" class="form-select" required>
                        @foreach(['non_urgente'=>'Non urgente','normale'=>'Normale','urgente'=>'Urgente','critica'=>'Critica'] as $key=>$label)
                            <option value="{{ $key }}" {{ old('urgenza')===$key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="mezzo_id" class="form-select">
                        <option value="">--</option>
                        @foreach($mezzi as $m)
                            <option value="{{ $m->id }}" {{ old('mezzo_id', $precompilazione['mezzo_id'])==$m->id ? 'selected' : '' }}>{{ $m->targa }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="dpi_id" class="form-select">
                        <option value="">--</option>
                        @foreach($dpi as $d)
                            <option value="{{ $d->id }}" {{ old('dpi_id', $precompilazione['dpi_id'])==$d->id ? 'selected' : '' }}>{{ $d->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="articolo_magazzino_id" class="form-select">
                        ...
                        @foreach($articoliMagazzino as $a)
                            <option value="{{ $a->id }}" {{ old('articolo_magazzino_id', $precompilazione['articolo_magazzino_id'])==$a->id ? 'selected' : '' }}>{{ $a->nome_articolo }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="mb-3">
                ...
                <input type="text" name="ubicazione_problema" class="form-control" value="{{ old('ubicazione_problema') }}">
            </div>
            <div class="form-check mb-2">
                <input type="checkbox" name="blocca_operativita" value="1" class="form-check-input" id="blocca" {{ old('blocca_operativita') ? 'checked' : '' }}>
                <label for="blocca" class="form-check-label">Blocca operatività</label>
            </div>
            <div class="form-check mb-3">
                <input type="checkbox" name="richiede_approvazione" value="1" class="form-check-input" id="approvazione" {{ old('richiede_approvazione') ? 'checked' : '' }}>
